---
tags: [aspnet, caching, rate-limiting, redis]
level: Senior
---
...
### Зачем?
- 100 пользователей запрашивают «популярные товары» → без кэша = 100 одинаковых SQL-запросов → БД перегружена
...
|----------|-----------|----------|-------|
| **IMemoryCache** | В памяти приложения | Наносекунды | Один сервер, некритичные данные |
| **IDistributedCache (Redis)** | Отдельный Redis | 1-5мс (сеть) | Несколько серверов, важные данные |
| **HybridCache (.NET 9)** | Сначала память, потом Redis | Лучшее из обоих | Новые проекты на .NET 9+ |
| **Output Cache** | Полный HTTP-ответ | Мгновенно | GET-запросы без персонализации |

### Что такое Rate Limiting?
Ограничение количества запросов от одного клиента за период времени. **Аналогия:** «Не более 10 человек одновременно» — табличка на двери магазина.

**Зачем:** Защита от DDoS, ботов, злоупотребления API.
