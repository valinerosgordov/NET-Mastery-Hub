...
См. [LLM / RAG patterns](llm-rag-patterns.md) — как сделать streaming endpoint, который отдаёт `IAsyncEnumerable`.
...
- [LLM / RAG patterns](llm-rag-patterns.md) — streaming `IAsyncEnumerable` в Blazor UI (NexusAI deck generation)
- [Pipeline и Middleware](../AspNetCore/pipeline-middleware.md) — порядок middleware для Blazor
- [Auth и Security](../AspNetCore/auth-security.md) — JWT, claims, policies (применимо к Blazor)
- [Resilience и HttpClient](../AspNetCore/resilience.md) — Polly для Blazor-клиентов API
- [Performance](../Performance/performance.md) — общие принципы для server-side Blazor
- [Testing](../Testing/testing.md) — bUnit как часть стека
