...
### Case Study #1 — Pipeline middleware для request logging

**Сценарий:** Нужно logging каждого HTTP request с timing.
...
### Case Study #2 — Tenant resolution

**Сценарий:** Multi-tenant SaaS. Tenant определяется из subdomain (`acme.myapp.com`).
...
public class TenantMiddleware(RequestDelegate next)
{
    public async Task InvokeAsync(HttpContext ctx, ITenantStore store)
    {
        ...
        var tenant = await store.FindByCodeAsync(subdomain);
        if (tenant is null)
        {
...
Downstream services читают `ctx.Items["Tenant"]`.

---

### Case Study #3 — Custom error handling

**Сценарий:** Все exceptions → consistent JSON response.
...
public class GlobalExceptionMiddleware(RequestDelegate next, ILogger<GlobalExceptionMiddleware> logger)
{
    public async Task InvokeAsync(HttpContext ctx)
    {
        ...
        {
            await WriteProblem(ctx, 422, "Validation failed", ex.Errors);
        }
        catch (NotFoundException ex)
        {
            await WriteProblem(ctx, 404, ex.Message);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unhandled exception");
            await WriteProblem(ctx, 500, "Internal server error");
        }
    }

    private async Task WriteProblem(HttpContext ctx, int status, string title, object? errors = null)
    {
        ctx.Response.StatusCode = status;
        ctx.Response.ContentType = "application/problem+json";
        await ctx.Response.WriteAsJsonAsync(new { title, status, errors });
    }
}
```
...
|--------------------|-------|
| ExceptionHandler | First (catches all) |
| HSTS | Early |
...
| Authorization | Before endpoints |
| Custom middleware | Position depends on logic |
| Endpoints | `app.MapControllers()` last |

| Pattern | When |
|---------|------|
| Conventional middleware (class) | Reusable, complex logic |
| Inline (`app.Use`) | Simple, one-time |
| Endpoint filter | API-specific, .NET 7+ |
| Action filter (controller) | MVC-specific behavior |
| Result filter | Response transformation |

| Common middlewares | Use |
|--------------------|-----|
| `UseExceptionHandler` | Catch + log + format |
| `UseRouting` | Match request → endpoint |
| `UseAuthentication` | Validate token / cookie |
| `UseAuthorization` | Check policies |
| `UseRateLimiter` | .NET 7+ throttling |
| `UseCors` | Cross-origin |
| `UseResponseCompression` | gzip/brotli |
| `UseResponseCaching` | HTTP caching |
| `UseOutputCache` | .NET 7+ output cache |
...
```
Какой ASP.NET Core component?
│
├── Cross-cutting concern для ALL requests?
│   ├── Class middleware → reusable, testable
│   └── Inline app.Use(...) → quick prototype
│
├── Specific endpoints / controllers?
│   ├── Action filter → MVC-only logic (auth check, model validation)
│   ├── Endpoint filter (.NET 7+) → Minimal API
│   └── Authorization policy → reusable rules
│
├── Modifying response?
│   ├── Headers / status code → middleware
│   ├── Body transformation → result filter
│   └── Content negotiation → AddControllers().AddXmlFormatters()
│
├── Where to handle exceptions?
│   ├── Global → IExceptionHandler (.NET 8+) или middleware
│   ├── Controller-level → exception filter
│   └── Specific endpoint → try/catch в action
│
└── Order matters?
    ├── HTTPS redirect → very early
    ├── Auth → before authorization
    ├── CORS → after routing
    └── Custom → think about WHO needs to see request first
```
