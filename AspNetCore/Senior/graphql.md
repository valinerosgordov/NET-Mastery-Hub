...
**Сценарий:** Admin panel показывает live KPIs из 5 микросервисов.

**❌ REST polling:** каждые 5 сек → 5 запросов × N админов = много нагрузки.

**✅ GraphQL subscription:**
```graphql
subscription DashboardUpdates {
  orderCreated { id total }
  paymentProcessed { id status }
  inventoryChange { productId stock }
}
```

Один WebSocket connection, server push на events.

См. [[signalr|SignalR]] для альтернативы.

---

### Case Study #2 — Mobile app — bandwidth optimization

**Сценарий:** Mobile app экран профиля. REST: 5 endpoints, 50 KB total. Mobile медленный network.

**REST:** 5 sequential calls, latency × 5.
...
query Profile {
  ...
  recommendations(top: 3) { id name price }
}
```

1 query, 8 KB response (only requested fields). 6x меньше data.

---

### Case Study #3 — N+1 проблема в GraphQL

**Сценарий:** Query orders + customer name → N+1.

**❌ Naive resolver:**
```csharp
public async Task<Customer> GetCustomer([Parent] Order order, ICustomerService svc) =>
    await svc.GetByIdAsync(order.CustomerId);
// 100 orders → 100 customer fetches!
```
...
| Need | GraphQL feature |
|------|-----------------|
| Library .NET | HotChocolate (recommended) |
| Query | `query { ... }` |
| Mutation (write) | `mutation { ... }` |
| Subscription (real-time) | `subscription { ... }` |
| Batch loading | DataLoader (avoid N+1) |
| Authorization | `@authorize` directive |
| Pagination | Relay cursor-based |
| Error handling | Errors array + extensions |
| Schema-first | SDL files (.graphql) |
| Code-first | C# classes + attributes |
| Performance | DataLoader, query cost analysis |
| Caching | Persisted queries, Apollo cache |

**REST vs GraphQL:**

| | REST | GraphQL |
|--|------|---------|
| Endpoints | Many | 1 (`/graphql`) |
| Over-fetching | Yes | No (request specific fields) |
| Under-fetching | Yes (multiple calls) | No (1 query) |
| Caching | HTTP cache | Persisted queries |
| Versioning | URL / header | Schema evolution |
| Discovery | OpenAPI | Introspection |
| Real-time | SSE / WebSocket | Subscriptions |
| Best for | Simple CRUD | Complex client needs |
...
│
├── Client needs?
│   ├── Mobile (bandwidth-sensitive) → GraphQL (specific fields)
│   ├── Web SPA (complex screens) → GraphQL
│   ├── Simple CRUD → REST
│   └── Public API (3rd party) → REST (более familiar)
│
├── Backend complexity?
│   ├── < 10 entities → REST OK
│   ├── 10+ + complex relations → GraphQL benefit
│   └── Microservices aggregation → GraphQL gateway
│
├── Real-time нужен?
│   ├── 1-way push notifications → SSE (REST + Server-Sent Events)
│   ├── Bi-directional → GraphQL subscriptions / SignalR
│   └── Streaming data → gRPC streaming
│
├── Team experience?
│   ├── REST familiar → стой с REST
│   ├── Want to learn → spike GraphQL на одном feature
│   └── Mixed → REST + GraphQL coexist (некоторые endpoints)
│
└── Performance concerns?
    ├── Caching critical → REST (HTTP cache friendly)
    ├── Specific fields important → GraphQL
    └── Latency critical → gRPC (binary, smaller)
```

**Hybrid approach:** REST для public API + GraphQL для internal / mobile.
