---
tags: [aspnet, auth, jwt, cors, security]
level: Senior
---
...
> **Cookie** — server-side state, CSRF-защита нужна, подходит для MVC/SSR. **JWT** — stateless, подходит для API/SPA/mobile, хранить в httpOnly cookie (не localStorage!). **Refresh tokens** — для ротации короткоживущих access token.

> [!question]- **Интервью: CORS — зачем и как?**
> Cross-Origin Resource Sharing. Браузер блокирует запросы к другому origin. Сервер должен ответить `Access-Control-Allow-Origin`. CORS — до Auth, чтобы preflight работал без токена.
