...
`dotnet-counters` — для ad-hoc. В prod лучше OpenTelemetry → Prometheus (см. [Observability](../Infrastructure/observability.md)).
...
См. подробнее в [Runtime / GC and Memory](../Runtime/gc-memory.md) и [HFT / Low-Latency](hft-low-latency.md).
...
См. подробнее в [Span, Memory, Layout](../Runtime/span-layout.md) и [HFT / Low-Latency](hft-low-latency.md).
...
В library code — да. В ASP.NET Core (без SyncContext) — необязательно. См. [Async и Threading](../CSharp/async-threading.md).
...
- [Span, Memory, Layout](../Runtime/span-layout.md) — Span, stackalloc, struct layout
- [GC, LOH, POH](../Runtime/gc-memory.md) — поколения, фрагментация
- [Concurrency и Atomics](../Runtime/concurrency-atomics.md) — lock-free patterns
- [Source Generators](../CSharp/source-generators.md) — почему source-gen быстрее reflection
- [Native AOT](../AspNetCore/native-aot.md) — AOT performance (cold start, memory)
- [EF Core Queries](../EFCore/queries-performance.md) — DB query optimization
- [Observability](../Infrastructure/observability.md) — production metrics, dotnet-monitor
