...
**Аналогия:** Вместо того чтобы ксерокопировать страницу книги (аллокация), ты просто указываешь пальцем на нужный абзац (Span). Книга одна, копий нет.
...
| Парсинг CSV: 1M строк → 1M аллокаций | Span: 0 аллокаций, тот же результат |
...
| **stackalloc** | Маленький буфер (< 512 байт) на стеке | Фиксированный размер, только в unsafe/Span |
...
**Правило:** используй zero-allocation только на hot path (вызывается 1000+ раз/сек). На cold path — читаемость важнее.
