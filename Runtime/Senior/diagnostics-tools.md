...
### Case Study #1 — Production memory leak

**Сценарий:** Web API после 24h работы — 8 GB RAM, OOM crashes.
...
# 3. Analyze в visual studio или dotnet-dump analyze
> dumpheap -stat
> dumpheap -mt <suspicious-type>
```

**Found:** static `Dictionary<string, byte[]>` где-то в коде, не cleared.
...
### Case Study #2 — High CPU без видимой причины

**Сценарий:** Service @ 100% CPU, но user load низкий.
...
# Open в SpeedScope или PerfView
# Видим — regex compilation в hot path
```

**Fix:** `[GeneratedRegex]` (compile-time).

---

### Case Study #3 — GC pressure в production

**Симптомы:** p99 latency spikes до 2 sec каждые 30 сек.

**Diagnostic:**
```bash
# Watch GC events live
dotnet-counters monitor --process-id <pid> System.Runtime[gc-heap-size,time-in-gc]

# Trace
dotnet-trace collect --process-id <pid> --providers Microsoft-DotNETCore-SampleProfiler
```

**Found:** Gen 2 collections frequent → large object heap fragmentation.

**Fix:**
- ArrayPool для buffers
- struct vs class
...
| Tool | Use |
|------|-----|
| **dotnet-counters** | Live metrics (CPU, GC, threadpool) |
| **dotnet-trace** | Performance profiling, sampling |
| **dotnet-dump** | Memory snapshots |
| **dotnet-stack** | Thread call stacks |
| **dotnet-symbol** | Download symbols |
| **dotnet-monitor** | Production-ready diagnostics endpoint |
| **dotnet-gcdump** | GC heap snapshots |
| **PerfView** | Windows ETW profiling, comprehensive |
| **dotTrace** | JetBrains profiler (commercial) |
| **dotMemory** | JetBrains memory profiler |
| **Application Insights** | Azure APM |
| **Datadog APM** | Cross-platform APM |

| Symptom | Tool |
|---------|------|
| Slow API | APM (App Insights / Datadog) |
| Slow specific method | dotnet-trace + sampling |
| High memory | dotnet-dump или dotMemory |
| High CPU | dotnet-trace |
| Memory leak | dotnet-dump diff snapshots |
| GC pauses | dotnet-counters [gc-heap-size, time-in-gc] |
| Thread pool starvation | dotnet-counters [threadpool] |
| Async deadlock | dotnet-stack threads |
...
│   ├── Quick check → dotnet-counters monitor
│   ├── Continuous → Application Insights / Datadog / Prometheus
│   └── Dashboards → Grafana over Prometheus
│
├── Performance issue?
│   ├── CPU bottleneck → dotnet-trace + speedscope
│   ├── Allocations / GC → dotnet-trace + GC events
│   ├── Specific method timing → BenchmarkDotNet
│   └── Hot path identified → PerfView для deep analysis
│
├── Memory issue?
│   ├── Live monitoring → dotnet-counters [gc-heap-size]
│   ├── Snapshot → dotnet-dump или dotnet-gcdump
│   ├── Find leak → 2+ snapshots + diff в dotMemory
│   └── Production safe → dotnet-monitor (HTTP endpoint)
│
├── Hung / deadlock?
│   ├── Stack traces all threads → dotnet-stack
│   ├── Async deadlock → debug build + async stack traces
│   └── Lock contention → dotnet-trace + Concurrency Visualizer
│
├── Crash analysis?
│   ├── Core dump → dotnet-dump analyze
│   ├── Need symbols → dotnet-symbol fetch
│   └── Deep investigation → WinDbg + SOS extension
│
├── Distributed tracing?
│   ├── Microservices → OpenTelemetry → Jaeger / Tempo
│   └── Cloud-native → Azure Monitor / AWS X-Ray
│
└── In-production diagnostics?
    ├── Safe always-on → dotnet-monitor sidecar
    ├── On-demand triggers → dotnet-monitor с triggers
    └── Custom metrics → Meter API + OpenTelemetry export
```
