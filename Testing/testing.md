...
**Аналогия:** Чек-лист перед вылетом самолёта. Пилот не «верит», что шасси работает — он проверяет по списку. Тесты — чек-лист для кода.
...
| Боишься рефакторить — вдруг что-то сломается | Рефакторишь смело — тесты страхуют |
| Баг в продакшене, клиент нашёл | Баг найден в тесте, до продакшена не дошёл |

### Когда какой тест?

| Тип теста | Что проверяет | Скорость | Когда |
|-----------|---------------|----------|-------|
| **Unit** | Один метод/класс в изоляции | Мс | Бизнес-логика, Value Objects, чистые функции |
| **Integration** | Связка компонентов (API → БД) | Сек | Endpoints, репозитории, EF запросы |
...
|--------|--------------|-------|
| **Пирамида** | Много unit → меньше integration → мало E2E | Сложная бизнес-логика (DDD) |
