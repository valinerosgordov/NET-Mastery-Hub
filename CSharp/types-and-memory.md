...
**Правила, как данные хранятся в памяти.** Каждая переменная имеет тип, который определяет: сколько памяти занимает, где хранится (стек или куча), как копируется.

**Аналогия:** Типы — это коробки разного размера. `int` — маленькая коробка (4 байта, лежит на столе = стек). `class` — большая коробка на складе (куча), а на столе только записка с адресом склада (ссылка).

### Зачем это знать?

| Без понимания | С пониманием |
|--------------|-------------|
| «Почему `struct` не меняется после передачи в метод?» | Value type копируется — метод работает с копией |
| «Откуда boxing и почему тормозит?» | `int` → `object` = аллокация в куче, нагрузка на GC |
| «Когда `class`, когда `struct`?» | Struct < 16 байт, immutable, короткоживущий. Иначе class |
| «Зачем `Span<T>`?» | Окно в память без копирования — zero-alloc парсинг |

### Когда что использовать?

| Тип | Когда | Пример |
|-----|-------|--------|
| **class** | Идентичность важна, сложный объект, наследование | `Order`, `Customer`, сервисы |
| **struct** | Маленький (< 16 байт), immutable, значение без идентичности | `Point`, `DateTime`, `Guid` |
| **record class** | DTO, Value Object, нужен `Equals` по значению | `OrderDto`, `Address` |
| **record struct** | Как struct, но с value equality из коробки | `Money(decimal Amount, string Currency)` |
| **Span\<T\>** | Парсинг, слайсинг массивов без аллокаций | Hot path, парсинг строк |
