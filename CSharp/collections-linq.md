...
### Что такое коллекции?
**Контейнеры для хранения набора объектов.** Каждый тип коллекции оптимизирован под определённые операции: поиск, вставка, порядок.

**Аналогия:** Коллекция — это шкаф. `List` — полка (по порядку). `Dictionary` — ящик с подписями (быстрый поиск по имени). `HashSet` — мешок уникальных предметов. `Queue` — очередь в магазине.

### Что такое LINQ?
**Language Integrated Query** — SQL-подобный синтаксис прямо в C#. Фильтрация, сортировка, группировка любых коллекций одним стилем.

### Когда какую коллекцию?

| Задача | Коллекция | Почему |
|--------|-----------|--------|
| Последовательный список, доступ по индексу | **List\<T\>** | O(1) по индексу, O(n) поиск |
| Быстрый поиск по ключу | **Dictionary\<K,V\>** | O(1) поиск, вставка, удаление |
| Уникальные элементы, проверка «содержит ли?» | **HashSet\<T\>** | O(1) Contains |
| Очередь (первый пришёл — первый вышел) | **Queue\<T\>** | FIFO |
| Стек (последний пришёл — первый вышел) | **Stack\<T\>** | LIFO |
| Неизменяемый словарь (read-only hot path) | **FrozenDictionary** | Быстрее Dictionary для чтения |
| Потокобезопасная коллекция | **ConcurrentDictionary** | Lock-free для многопоточности |
| Только перечисление (lazy) | **IEnumerable\<T\>** | Отложенное выполнение, yield |
