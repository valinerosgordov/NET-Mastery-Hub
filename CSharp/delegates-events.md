...
### Что такое делегат?
**Переменная, которая хранит метод.** Можно передавать методы как параметры, хранить в коллекциях, вызывать позже.

**Аналогия:** Делегат — это записка «позвони Маше по номеру 555-1234». Ты не звонишь сам — передаёшь записку (делегат) кому-то, и он позвонит когда нужно.

### Что такое событие (event)?
**Уведомление «что-то произошло».** Один объект кричит «заказ создан!», другие слушают и реагируют. Publisher-Subscriber паттерн.

### Что такое лямбда?
**Анонимный метод в одну строку.** Вместо `private bool IsAdult(Person p) { return p.Age >= 18; }` → `p => p.Age >= 18`.

### Когда что использовать?

| Конструкция | Когда | Пример |
|-------------|-------|--------|
| **Action\<T\>** | Метод без возврата (void) | Callback: `onComplete: () => Log("done")` |
| **Func\<T, TResult\>** | Метод с возвратом | LINQ: `.Where(x => x.Age > 18)` |
| **Event** | Уведомление подписчиков (1 → N) | `OrderCreated`, `ButtonClicked` |
| **Лямбда** | Короткий inline метод | `.Select(x => x.Name)` |
| **Delegate** | Кастомная сигнатура (редко) | Обычно хватает Action/Func |
