---
tags: [async, await, tasks, threading, synchronization, channels]
level: Senior
---

# Async, Threading и Concurrency

## Что это, зачем и когда

### Что такое async/await?
**Способ не блокировать поток во время ожидания.** Пока ждёшь ответ от БД / HTTP — поток свободен для других запросов.

**Аналогия:** Ты заказал пиццу по телефону. **Sync** = стоишь у двери и ждёшь курьера (ничего не делаешь). **Async** = заказал, пошёл заниматься делами, курьер позвонит когда приедет.

### Зачем?

| Без async | С async |
|-----------|---------|
| 100 потоков = 100 одновременных запросов (потом — очередь) | 100 потоков = тысячи одновременных запросов (потоки переиспользуются) |
| Поток спит, пока ждёт БД (1 GB RAM на 1000 потоков) | Поток свободен, обслуживает другие запросы |
| UI замирает на время загрузки | UI отзывчив — загрузка в фоне |

### Когда что?

| Задача | Подход | Почему |
|--------|--------|--------|
| Вызов БД, HTTP, файл | **async/await** | I/O-bound — поток не нужен во время ожидания |
| Тяжёлые вычисления (CPU) | **Task.Run** | CPU-bound — нужен отдельный поток |
| Фоновая очередь задач | **Channel\<T\> + BackgroundService** | Producer-consumer без блокировки |
| Несколько независимых запросов | **Task.WhenAll** | Параллельно, не последовательно |
| Общий ресурс из нескольких потоков | **SemaphoreSlim / Lock** | Синхронизация доступа |
| Таймаут операции | **CancellationToken** | Отмена через `.WithCancellation(ct)` |

---

> Справочник по асинхронности, многопоточности и concurrency. C# 13 / .NET 9.
...
> [!question]- **Интервью: Что async/await делает под капотом?**
> Компилятор генерирует state machine (`IAsyncStateMachine`). Каждый `await` — точка приостановки. State machine сохраняет локальные переменные и состояние. При завершении awaited Task — continuation через `SynchronizationContext` или `TaskScheduler`.
>
> **Ключевое:** async не создаёт новый поток. Для I/O-bound — поток возвращается в ThreadPool. Для CPU-bound — нужен `Task.Run()`.
...
> [!question]- **Интервью: ConfigureAwait(false) — когда и зачем?**
> Не захватывать `SynchronizationContext` после await. Continuation на любом потоке ThreadPool.
>
> **Библиотеки:** всегда `ConfigureAwait(false)`. **ASP.NET Core:** нет SyncContext, но хорошая привычка. **WPF/WinForms:** без него continuation на UI-потоке, с ним — может упасть при обращении к UI.
...
> [!question]- **Интервью: Почему async void — зло?**
> 1. Исключения не перехватываются — падают в `SynchronizationContext` (crash)
> 2. Невозможно await — нельзя дождаться завершения
> 3. Не compose — не участвует в `Task.WhenAll`
>
> **Единственное исключение:** event handlers (`async void Button_Click`).
...
> [!question]- **Интервью: Task vs ValueTask — когда что?**
> **Task** — по умолчанию, безопасный, можно await несколько раз, кешировать.
>
> **ValueTask** — оптимизация для hot path, когда результат часто синхронный (кеш). Экономит аллокацию.
>
> **Ограничения ValueTask:** нельзя await дважды, нельзя `.Result` до завершения, нельзя хранить. Только high-performance сценарии.

> [!question]- **Интервью: Что с исключениями в Task.WhenAll?**
> `Task.WhenAll` дожидается **всех** задач. При await бросает **первое** исключение. Остальные — в `task.Exception.InnerExceptions`.
...
- [Concurrency и атомарность](../Runtime/concurrency-atomics.md)
