...
### Что такое Exception?
**Сигнал «что-то пошло не так».** Прерывает нормальный поток, передаёт управление в catch-блок. Содержит сообщение, стектрейс, тип ошибки.

**Аналогия:** Пожарная сигнализация. Работа останавливается, все реагируют на тревогу. Дорого (стектрейс = расследование), поэтому не дёргай без повода.

### Когда что использовать?

| Ситуация | Подход | Почему |
|----------|--------|--------|
| БД упала, сеть недоступна | **Exception** | Инфраструктурный сбой — исключительная ситуация |
| Email уже занят, невалидный ввод | **Result Pattern** | Ожидаемая ошибка — не исключение, а бизнес-логика |
| Метод может вернуть null | **Nullable + ?. ??** | `Order? order = Find(id); order?.Cancel();` |
| Парсинг строки в число | **TryParse** | `int.TryParse(s, out var n)` — без exception |
| Файл не найден, JSON битый | **try/catch конкретного типа** | `catch (FileNotFoundException)` — не голый `catch` |

### Строки — когда что?

| Задача | Инструмент | Почему |
|--------|-----------|--------|
| Склеить 2-3 строки | **Интерполяция** `$"Hello {name}"` | Читаемо, быстро |
| Склеить в цикле (100+ раз) | **StringBuilder** | String immutable → каждая конкатенация = новый объект |
| Многострочный текст / JSON | **Raw string literals** `"""..."""` | Без экранирования |
| Парсинг без аллокаций | **Span\<char\>** | Hot path, zero-alloc |
