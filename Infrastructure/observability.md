---
tags: [opentelemetry, jaeger, seq, observability, tracing]
level: Senior
---
...
Способность **видеть**, что происходит внутри работающего приложения. Логи, метрики, трейсы — три инструмента, которые показывают **где** тормозит, **почему** падает, и **сколько** ресурсов тратится.

**Аналогия:** Бортовой самописец («чёрный ящик») в самолёте. Записывает ВСЁ: скорость, высоту, действия пилота. Если что-то пошло не так — можно восстановить картину.

### Зачем нужно?

| Без observability | С observability |
|---|---|
| «Сайт тормозит» → «Не знаю почему» → «Давайте перезагрузим» | «Сайт тормозит» → Grafana: «DB latency 2с» → Jaeger: «fullscan в GetOrders» → фикс за 5 мин |
| Баг в продакшене → не воспроизводится локально | Seq: видим ТОЧНЫЕ логи с контекстом на момент ошибки |
| «Сколько заказов в час?» → «Не знаю» | Prometheus: метрика `orders_created_total` → дашборд в Grafana |

### Когда использовать?
- **Всегда в production** — без observability ты слепой
- **С первого дня проекта** — подключить OpenTelemetry занимает 15 минут
- **Обязательно для микросервисов** — без трейсов невозможно понять путь запроса
