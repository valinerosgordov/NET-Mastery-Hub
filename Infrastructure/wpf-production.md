...
- [Async и Threading](../CSharp/async-threading.md) — `IProgress<T>`, `ConfigureAwait`, Dispatcher pitfalls
- [DI и Configuration](../AspNetCore/di-configuration.md) — Generic Host, Options pattern (применимо и к WPF)
- [HFT / Low-Latency](../Performance/hft-low-latency.md) — когда WPF UI-thread vs hot-path threads (для TradingBotForex)
- [WPF ViewModel snippet](../Snippets/wpf-viewmodel.md) — короткие готовые сниппеты Toolkit'а
- [Testing](../Testing/testing.md) — как тестировать ViewModel без WPF-thread (через IDispatcher abstraction)
