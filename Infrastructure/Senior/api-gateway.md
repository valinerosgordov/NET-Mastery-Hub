...
```json
"orders-cluster": {
  "LoadBalancingPolicy": "PowerOfTwoChoices",  // RoundRobin | Random | LeastRequests | PowerOfTwoChoices | FirstAlphabetical
  "Destinations": {
...
Политика задаётся **per-cluster** через `LoadBalancingPolicy` — у каждого кластера может быть своя. Имя policy — строка; неизвестное значение валится на старте, так что опечатку поймаешь сразу.

### Почему выбор политики важен

Дефолт по умолчанию (если ничего не указать) — `PowerOfTwoChoices`. Но понимать, **почему**, важнее, чем запомнить дефолт: на гетерогенной нагрузке (разные по цене запросы, destinations разной мощности, "warm" vs "cold" инстансы) неправильная политика раздувает p99, даже когда средняя нагрузка низкая.

#### RoundRobin / Random — распределяют, не балансируют

Обе раскидывают запросы **вслепую**, не глядя на текущую загрузку destination:

- **RoundRobin** — по кругу: 1 → 2 → 3 → 1 → …
- **Random** — равномерно случайно.

Проблема одна: если запрос #5 ушёл на медленный/занятый узел и тот ещё его перемалывает, RoundRobin всё равно через круг пришлёт туда #8. На однородной нагрузке (все запросы дёшевы, все узлы равны) это нормально и почти бесплатно. На гетерогенной — запросы **наваливаются на занятые узлы**, очередь на них растёт, и хвост латентности (p99) распухает, хотя средние метрики выглядят здоровыми.

#### LeastRequests — лучший хвост, но дороже на масштабе

Маршрутизирует на destination с **наименьшим числом in-flight запросов** (тех, что прямо сейчас в обработке). Занятый узел перестаёт получать новые запросы, пока не разгребёт текущие — именно это режет p99 на неоднородной нагрузке.

Цена: чтобы выбрать минимум, нужно на **каждый** запрос просканировать счётчики in-flight по **всем** destinations. На 3 узлах это шум; на 50–100 узлах сканирование всего пула на каждый запрос становится заметной статьёй расхода в hot path прокси.

#### PowerOfTwoChoices — почти LeastRequests за `O(1)`

Сэмплит **две случайные** destinations и отправляет запрос в ту, у которой меньше in-flight. Не сканирует весь пул — стоимость не зависит от числа узлов (`O(1)` против `O(n)` у LeastRequests).

Магия в том, что выбор лучшего из двух случайных почти полностью устраняет "наваливание", которым страдают RoundRobin/Random: вероятность, что обе выборки попадут на уже перегруженные узлы, мала. Качество балансировки — близко к LeastRequests, цена — как у Random. Поэтому это **прагматичный дефолт для больших кластеров**.

#### FirstAlphabetical — детерминированный пиннинг

Всегда шлёт на первую по алфавиту **здоровую** destination; на следующую переключается, только когда первая выпала по health check. Это не "балансировка" в смысле размазывания нагрузки — это **детерминированный приоритет**: active/standby (горячий резерв), осознанный primary-инстанс, либо стабильность кэш-локальности (один и тот же destination → попадания в его кэш). Для размазывания нагрузки не подходит.

### Комбинация с health checks

Load balancing работает **поверх** результата health checks (см. ниже): балансировщик выбирает только среди destinations, помеченных как healthy. Мёртвый узел сначала выпадает из пула по health check, и только потом LB-политика раскидывает трафик по оставшимся.

Без health checks любая политика будет уверенно слать часть запросов на мёртвый узел: PowerOfTwoChoices увидит у него `0` in-flight (он же ничего не обрабатывает) и сочтёт его **самым свободным** — идеальным кандидатом. Связка обязательна: health checks убирают мёртвых, LB-политика балансирует живых.

### Трейдофы политик

| Policy | Стоимость выбора | p99 на гетерогенной нагрузке | Когда брать |
|--------|------------------|------------------------------|-------------|
| RoundRobin | `O(1)` | плохо (наваливание на занятых) | однородные запросы и узлы, нужен предсказуемый порядок |
| Random | `O(1)` | плохо (наваливание на занятых) | однородная нагрузка, простота, отсутствие состояния |
| LeastRequests | `O(n)` по пулу | лучший | мало destinations, хвост латентности критичен |
| PowerOfTwoChoices | `O(1)` (2 сэмпла) | почти как LeastRequests | **дефолт для больших кластеров** |
| FirstAlphabetical | `O(1)` | n/a (не балансирует) | active/standby, пиннинг на primary, кэш-локальность |
