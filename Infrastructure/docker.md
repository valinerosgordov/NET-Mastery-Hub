...
| Distroless `cc-debian12` + AOT | 18 MB | См. [Native AOT](../AspNetCore/native-aot.md) |
...
См. [Native AOT](../AspNetCore/native-aot.md) — distroless + AOT = 18 MB image.
...
`preStop` выполняется **перед** SIGTERM. `sleep 5` даёт kube-proxy время удалить pod из endpoints. Иначе race: SIGTERM пришёл, мы перестали accept'ить, но balancer ещё шлёт. См. [Kubernetes](kubernetes.md).
...
В Kubernetes — `kubectl rollout` делает это автоматически. См. [Kubernetes](kubernetes.md).
...
Размер: 220 MB → 60-80 MB. **Ломает reflection**, нужны annotations. См. [Native AOT](../AspNetCore/native-aot.md).
...
Размер: 60 MB → 20-25 MB. + быстрый cold start (10x), низкая память (3x). Подробно: [Native AOT](../AspNetCore/native-aot.md).
...
**Решение:** в-flight request draining + pre-stop sleep + IHostApplicationLifetime. См. [Kubernetes](kubernetes.md).
...
- [Native AOT](../AspNetCore/native-aot.md) — distroless + AOT для extra-small images
- [Kubernetes](kubernetes.md) — orchestration, probes, graceful shutdown под .NET
- [Observability](observability.md) — health checks, metrics, OpenTelemetry в containerized
- [Resilience](../AspNetCore/resilience.md) — что делать когда container перезапустился
- [Project Setup](project-setup.md) — Directory.Build.props + Dockerfile setup
- [Testing](../Testing/testing.md) — Testcontainers
- [GC и память в контейнерах](../Runtime/gc-memory.md) — DATAS, cgroup awareness, heap limits
- [System Design](../Architecture/system-design.md) — Docker как deployment layer
- [HFT/Low-Latency](../Performance/hft-low-latency.md) — почему контейнеры могут давать latency spikes (CPU throttling)
