...
### Что такое Docker?
Docker — «коробка», в которую упаковываешь приложение **вместе со всем**, что ему нужно (runtime, библиотеки, настройки). Эта коробка работает **одинаково** на любом компьютере — твоём, коллеги, сервере.

**Аналогия:** Ты переезжаешь. Без Docker — таскаешь вещи по одной (забудешь, сломаешь). С Docker — складываешь ВСЁ в контейнер и перевозишь целиком.
...
| На сервере другой .NET / PostgreSQL | Dockerfile описывает ТОЧНУЮ версию всего |
| Нужно ставить PostgreSQL, Redis на свой комп | `docker compose up` — всё поднимается одной командой |
| Деплой — ручная настройка сервера | Деплой — запуск готового образа |
| «Забыл установить Redis на сервер» | `docker-compose.yml` описывает ВСЮ инфраструктуру |

### Когда использовать?
- **Деплой на сервер** — ВСЕГДА. Предсказуемость окружения
- **Локальная разработка** — не ставить PostgreSQL/Redis/RabbitMQ на свой комп
- **CI/CD** — тесты в изолированном окружении
...
|---------|---------|----------|
| **Image** (образ) | Шаблон/рецепт для контейнера | Чертёж дома |
| **Container** | Запущенный экземпляр образа | Построенный дом по чертежу |
| **Dockerfile** | Инструкция для создания образа | Пошаговый рецепт |
| **docker-compose** | Описание ВСЕЙ инфраструктуры | План посёлка (несколько домов) |
| **Volume** | Внешний диск для контейнера | Гараж, который не сносится при перестройке дома |
| **Port mapping** | Проброс порта наружу | Дверь, через которую можно войти |
| **Multi-stage build** | Сборка в одном образе, запуск в другом | Строители уходят, жильцы остаются |
