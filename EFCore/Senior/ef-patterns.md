...
### Case Study #1 — Repository pattern с EF

**Зачем:** Изолировать Domain от EF Core, тестировать без DB.
...
public class OrderRepository(AppDbContext db) : IOrderRepository
{
    ...
    public Task<List<Order>> GetByUserAsync(int userId) =>
        db.Orders.AsNoTracking().Where(o => o.UserId == userId).ToListAsync();

    public Task AddAsync(Order order) => db.Orders.AddAsync(order).AsTask();
}
```

> [!info] Anti-pattern alarm
> EF Core itself — already a repository. Generic `IRepository<T>` поверх EF — over-engineering. Используй specific repos с domain-meaningful методами (`GetActiveOrders` not `Get`).

---

### Case Study #2 — Specification pattern

**Сценарий:** Сложные queries с reusable filters.

```csharp
public class ActiveOrdersForUserSpec : Specification<Order>
{
    public ActiveOrdersForUserSpec(int userId, DateTime since)
    {
        Query.Where(o => o.UserId == userId && o.Status == "Active" && o.CreatedAt >= since);
...
// Use
var spec = new ActiveOrdersForUserSpec(userId, DateTime.UtcNow.AddDays(-30));
var orders = await _repo.ListAsync(spec);
```

Используй пакет **Ardalis.Specification** — battle-tested.

---

### Case Study #3 — Unit of Work (built-in в EF)

EF Core's `DbContext` — это уже Unit of Work. Не оборачивай дополнительно!
...
public async Task TransferMoney(int fromId, int toId, decimal amount)
{
    ...
    await _db.SaveChangesAsync();  // single transaction — UoW commit
}
```

Если нужен **explicit transaction** (multi-DB ops):
```csharp
using var tx = await _db.Database.BeginTransactionAsync();
try
{
    await _db.SaveChangesAsync();
    await _otherDb.SaveChangesAsync();
...
|---------|---------------------|
| Repository | Хочешь testability + domain isolation |
| Specification | Сложные reusable queries |
| Unit of Work | Уже есть! `DbContext.SaveChangesAsync()` |
| CQRS | Read и Write модели сильно отличаются |
| Soft Delete | Audit / compliance / undelete |
| Audit log | Track who/when/what changed |
| Outbox pattern | Reliable event publishing |
| Saga | Distributed transactions |

**Anti-patterns:**
- ❌ `IRepository<T>` generic поверх EF (overengineering)
- ❌ Domain entities = EF entities (ломает Clean Architecture)
- ❌ Lazy loading в API responses (N+1!)
- ❌ Repository hides query capability (нельзя custom WHERE)
- ❌ DbContext per request (correctly!) но передаётся в services как singleton (BAD)
...
│
├── Tests без real DB?
│   └── Repository pattern (с interface)
│
├── Сложные queries reused в нескольких местах?
│   └── Specification pattern (Ardalis.Specification)
│
├── Read != Write модели?
│   └── CQRS (отдельные read/write contexts)
│
├── Audit / compliance?
│   ├── Soft delete (IsDeleted flag) + global query filter
│   └── Audit log (отдельная таблица + interceptor)
│
├── Reliable events publishing?
│   └── Outbox pattern (events в той же transaction что domain changes)
│
