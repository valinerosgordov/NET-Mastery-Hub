---
tags: [efcore, tracking, loading, dbcontext]
level: Senior
---

# Loading, Tracking и DbContext
...
**Аналогия — переводчик:** Ты говоришь «дай все заказы дороже 1000». EF переводит в `SELECT * FROM Orders WHERE Total > 1000`. Ты не пишешь SQL руками.

### Что такое DbContext?
«Сессия работы с БД». Открывает соединение, отслеживает изменения, генерирует SQL при `SaveChangesAsync()`.

**Важно:** DbContext = **Scoped** (один на HTTP-запрос). НЕ Singleton (утечка памяти, потокобезопасность).
...
| Режим | Зачем | Когда |
|-------|-------|-------|
| **Tracking** (по умолчанию) | EF следит за изменениями | Читаешь данные, чтобы **изменить** и сохранить |
| **AsNoTracking** | EF НЕ следит, быстрее на 30-50% | Читаешь данные **только для отображения** (GET) |

### Что такое Loading (загрузка связанных данных)?

| Тип | Как | Когда |
|-----|-----|-------|
| **Eager Loading** (`Include`) | Один SQL с JOIN | Знаешь заранее, что нужны связанные данные |
| **Explicit Loading** (`Entry().Collection().LoadAsync()`) | Отдельный SQL по требованию | Нужны связанные данные условно |
| **Lazy Loading** (прокси) | SQL при обращении к свойству | **НЕ рекомендуется** — N+1 ловушка |
| **Проекция** (`Select`) | Только нужные столбцы | **Лучший вариант** для read-only |

---

> [!question]- **Интервью: Change Tracker — назначение и влияние на производительность?**
> Change Tracker отслеживает состояние сущностей (Added, Modified, Deleted, Unchanged). При `SaveChanges()` генерирует SQL только для изменённых. Накладные расходы: snapshot каждой tracked сущности, DetectChanges() при каждом SaveChanges. Для read-only — `AsNoTracking()` убирает overhead.

> [!question]- **Интервью: Cartesian explosion — проблема и решение?**
> Множественные `Include` с коллекциями → декартово произведение в SQL (N×M строк). Решение: `AsSplitQuery()` — EF генерирует несколько SQL-запросов вместо одного JOIN. Или проекция (Select) — лучший подход для read-only.
