---
tags: [efcore, patterns, repository, tph, soft-delete, cqrs]
level: Senior
---
...
### Обзор паттернов EF Core

| Паттерн | Что делает | Когда нужен |
|---------|-----------|-------------|
| **Soft Delete** | Помечает «удалённым» вместо физического DELETE | Нужен аудит, восстановление, юридические требования |
| **TPH/TPT/TPC** | Стратегии маппинга наследования в таблицы | Иерархия типов в домене (Payment → CreditCard, BankTransfer) |
| **Repository** | Абстракция над DbContext | Тестируемость, замена ORM (редко нужно) |
| **Unit of Work** | Группа изменений сохраняется атомарно | Несколько репозиториев меняют данные в одном запросе |
| **Interceptors** | Хуки на операции EF (SaveChanges, Query) | Автоматический аудит (CreatedAt, UpdatedBy) |
| **Value Converters** | Маппинг C#-типа ↔ SQL-тип | Enum → string, DateTime → UTC, шифрование |
| **Global Query Filters** | Автоматический WHERE для всех запросов | Soft Delete (IsDeleted = false), Multi-tenancy (TenantId = X) |
